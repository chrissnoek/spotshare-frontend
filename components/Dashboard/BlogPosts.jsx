				...
				return (
					<div>
						<div className="flex justify-center items-center mb-4 cursor-pointer">
							<a
								className="mr-4 rounded flex-none cursor-pointer"
								href={`https://spotshare.nl${post.url}`}
								target="_blank"
							>
								<img
									className="rounded h-20 w-20 object-cover cursor-pointer"
									alt={post.title}
									src={`https://spotshare.nl${post.hero_image}`}
								/>
							</a>
							<a
								className="text-sm font-bold w-full cursor-pointer break-word"
								href={`https://spotshare.nl${post.url}`}
								target="_blank"
							>
								{post.title}
